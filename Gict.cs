...
namespace Gict
{
    public class Runtime
    {
        ...
        public static void check()
        {
            for (int count = 0; count < args.Length; count++)
            {
                if (args[count] == "print[")
                {
                    ...
                    {
                        Console.WriteLine("\nGICT - Print Statement Not Properly Ended\nTraceback:\n    Expected ']' At End Of Print Statement.");
                        break;
                    }
                }
                ...
                if (args[count] == "println[")
                {
                    ...
                    {
                        Console.WriteLine("\nGICT - Println Statement Not Properly Ended\nTraceback:\n    Expected ']' At End Of Println Statement.");
                        break;
                    }
                }
                ...
                if (args[count] == "((")
                {
                    ...
                    {
                        Console.WriteLine("\nGICT - Comment Not Properly Ended\nTraceback:\n    Expected '))' At End Of Comment.");
                        break;
                    }
                }
            }
        ...
        {
            var scriptfile = Glint.Kernel.fs.GetFile(script);
            var script_stream = scriptfile.GetFileStream();

            if (script_stream.CanRead)
            {
                byte[] text_to_read = new byte[script_stream.Length];
                script_stream.Read(text_to_read, 0, (int)script_stream.Length);
                Runtime.Interpreter(Encoding.Default.GetString(text_to_read));
            }
        }
    }
}